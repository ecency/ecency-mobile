...
export const checkClient = async () => {
  ...
  setHiveTxNodes(selectedServer, 4000);
};
...
export const sendHiveOperations = async (
  ...
  try {
    ...
    // broadcastOperations signs with the first key; sign with remaining keys manually
    // For single-key case (vast majority), this is a direct call
    if (keys.length === 1) {
      const result = await broadcastOperations(operations, keys[0]);
      return result;
    }
    // Multi-key: build transaction manually to sign with all keys
...
export const resolveTransaction = async (parsedTx, parsedParams, signer) => {
  ...
  const { tx } = hiveuri.resolveTransaction(parsedTx, parsedParams, {
    ...
    expiration: new Date(Date.now() + EXPIRE_TIME + EXPIRE_TIME).toISOString().slice(0, -5),
    signers: [signer],
