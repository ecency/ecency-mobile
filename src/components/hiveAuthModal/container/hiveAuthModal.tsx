...
export const HiveAuthModal = forwardRef(({ onClose }: HiveAuthModalProps, ref) => {
  ...
  const _renderProcessContent = (
    <>
      {status === Status.SUCCESS && _renderResultIcon('checkcircleo', '$primaryGreen')}
      {status === Status.ERROR && _renderResultIcon('closecircleo', '$primaryRed')}
  ...
  const _renderContent = (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        {status !== Status.IDLE ? _renderProcessContent : mode === Modes.AUTH && _renderAuthContent}
      </View>
    </View>
  );
